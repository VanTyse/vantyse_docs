...
function Home() {
    ...
    const getDocuments = async () => {
        ...
        try{
            const {data: documents} = await axios('/api/v1/documents', {
                headers: {
        ...
        <div className='home-container container'>
            <div className="documents">
            {
                ...
                documents.map(document => {
                    const {documentName, documentID, _id, updatedAt, collaborators} = document
                    return <DocumentCard key={_id} title={documentName} 
                    date={updatedAt} documentID={documentID} 
...
function DocumentCard({title, date, documentID, collaboratorsNo}){
    ...
    return(
        <>
        <div onClick={() => navigate(`/documents/${documentID}`)} className="document-card">
            <div className="top">                
            ...
            <div className="bottom">
                ...
                <div className="right">
                    ...
                    }} className="far fa-handshake handshake-icon"></i>
        ...
        {
            showCollaboratorModal && <AddCollaboratorModal documentID={documentID} 
            documentName={title}
...
function NewDocumentButton({setShowModal}){
    ...
    return(
        <div onClick={() => setShowModal(true)} className={`new-doc-btn ${direction === 'down' && 'hide-btn'}`}><span>+</span></div>
    )
}
    ...
    return(
        <div className={`new-document-modal ${showModal && 'show-modal'}`}>
            ...
            <form onSubmit={e => e.preventDefault()} action="">
                ...
                placeholder="Document's name" value={name}
...
function AddCollaboratorModal({showModal, setShowModal, documentID, documentName}){
    ...
    const handleAdd = async() => {
        try {
            ...
            const token = localStorage.getItem('token')
            const {data} = await axios.patch(`/api/v1/documents/${documentID}`, {collaboratorEmail: email}, {
                headers:{
                    authorization: `Bearer ${token}`
                }
            })
    ...
    return(
        <div className={`add-collaborator-modal ${showModal && 'show-modal'}`}>
            {alertMessage && <em><h5 style={{color: 'var(--dark-main-color)', textAlign: 'center'}} className="alert">{alertMessage}</h5></em>}
            <div className="top">
                <h3>Add Collaborator</h3>
                <h5>{documentName}</h5>
            </div>
            <form onSubmit={e => e.preventDefault()} action="">
                <input type="email" name="collaborator" id="collaborator-input" 
                placeholder="collaborator email" value={email}
