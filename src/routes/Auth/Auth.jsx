...
export const BASE_URL = "https://vantyse-docs-api.onrender.com";
...
function Auth() {
  ...
  const getUser = async () => {
    ...
    const { data } = await axios.get(`${BASE_URL}/api/v1/users/${userID}`, {
      headers: {
  ...
  return (
    <div className="login-container">
      ...
      {alert.show && (
        <Alerts show={alert.show} msg={alert.msg} type={alert.type} />
      )}
    </div>
  );
}
...
function RegisterForm({ setIsLoginOrRegister, getUser, setAlert }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      //   error.response.data.error.keyPattern.email == 1
      // ) {
      //   setAlert({
      //     show: true,
